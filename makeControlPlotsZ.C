...
void makeControlPlotsZ(TString ofname = "assemblerZ.root",int step=1, std::string product="BASICCALOJETS1PT20MATCHED") {
  ...
  std::string varexp = "MET[0]{0,1200,24,\"MET\"}:HLTHT[0]{0,1500,30,\"HT\"}:NBASICCALOJETSMATCHED[0]{0,8}:NBASICCALOJETS1MATCHED[0]{0,8}:NBASICCALOJETS1PT20MATCHED[0]{0,8}:NALLCALOJETS[0]{0,10}:MT[0]{0,200,20,\"MT\"}:WPT[0]{0,200,20,\"WPT\"}:(NGOODMUONS[0]+NGOODELECTRONS[0]){0,5,5,\"NGOODLEPTONS\"}:NINCLUSIVETAGGEDCALOJETSMATCHED[0]{0,10}:NINCLUSIVETAGGEDCALOJETS[0]{0,10}:SSPH[0]{0,1,20,\"SSPH\"}:HSPH[0]{0,1,20,\"HSPH\"}:LSPH[0]{0,1,20,\"LSPH\"}:stupakR[0]{0,0.4,50,\"STUPAKR\"}:stupakR2[0]{0,0.4,50,\"STUPAKR2\"}:Min$(abs(LEPTON_DPHI)){0,3.142,32,\"MINLEPTONDPHI\"}:Max$(abs(LEPTON_DPHI)){0,3.142,32,\"MAXLEPTONDPHI\"}:LRM[0]{0,3,60,\"LRM\"}:NINCLUSIVETAGGEDCALOJETSE[0]{0,5}:NINCLUSIVETAGGEDCALOJETSF[0]{0,5}:M_INCLUSIVETAGGEDCALOJETSE[0]{0,100}:MASSDISPLACED_INCLUSIVETAGGEDCALOJETSE[0]{0,100}:ONZ{0,1}:PTOSSF[0]{0,200,20,\"PTOSSF\"}:MLL[0]{0,150,15,\"MLL\"}";
	...
	TString selection = "hasGoodVertex && (NGOODMUONS[0] == 2 || NGOODELECTRONS[0] == 2) && NBASICCALOJETS1PT20[0] > 0 && ((NGOODMUONS[0] > 0 && MUON_PT[0] > 23) || (NGOODELECTRONS[0] > 0 && ELECTRON_PT[0] > 31)) && NGOODLEPTONS[0] == 2";
	// Cut down charge flips
	// commented out because NPOSGOODMUONS etc. is not available in tcH ntuple	


	PhysicsContribution* signal1 = new PhysicsContribution("signal","./addedHistos/allHistos_ZH_HToSSTobbbb_ZToLL_MH-125_MS-55_ctauS-10_TuneCUETP8M1_13TeV-powheg-pythia8_0.root",0.8696*0.577*0.102*0.2,"ZH m_s = 55, ctau = 1 cm",false,"treeR",kGreen+2,0.001);
	PhysicsContribution* signal2 = new PhysicsContribution("signal","./addedHistos/allHistos_ZH_HToSSTobbbb_ZToLL_MH-125_MS-15_ctauS-1000_TuneCUETP8M1_13TeV-powheg-pythia8_0.root",0.8696*0.577*0.102*0.2,"ZH m_s = 15, ctau = 100 cm",false,"treeR",kRed+3,0.001);
	...
	if(!assembler){
	  ...
	  assembler->addContribution(signal1); //********** // It is important to add the signal before setting up the rake rates
	  assembler->addContribution(signal2); //********** // It is important to add the signal before setting up the rake rates
	  assembler->addBundle(new Bundle("signal", "signal1",false,kGreen+2));
	  assembler->getBundle("signal1")->addComponent(signal1);
	  assembler->addBundle(new Bundle("signal", "signal2",false,kRed+3));
	  assembler->getBundle("signal2")->addComponent(signal2);
	...
	std::string fileStart=std::string(TString::Format("Z_set%i_%s_",step,product.c_str()).Data());
	...
	makePlotsZ(assembler,fileStart,product,step);
	ofile.Close();
